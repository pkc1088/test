...
public:
    ...
    void search(float minKey, float maxKey, vector<long>& rst);
private:
    BPTnode* rt;
    ...
    void branchLeaf(BPTnode* lf, BPTnode** newLf, float* newKey);
};
...
void BPT::branchLeaf(BPTnode* lf, BPTnode** newLf, float* newKey) {
    *newLf = new BPTnode(true);
    int mid = lf->keys.size() / 2;
    *newKey = lf->keys[mid];
    (*newLf)->keys.assign(lf->keys.begin() + mid, lf->keys.end());
    (*newLf)->offsets.assign(lf->offsets.begin() + mid, lf->offsets.end());
    lf->keys.resize(mid);
...
void BPT::search(float minKey, float maxKey, vector<long>& rst) {
    BPTnode* cur = rt;
    while (!cur->isLf) {
        int i;
    ...
    while (cur) {
        for (int i = 0; i < cur->keys.size(); i++) 
            if (cur->keys[i] >= minKey && cur->keys[i] <= maxKey) rst.push_back(cur->offsets[i]);
        cur = cur->chdr.empty() ? nullptr : cur->chdr.back();
    }
}
unordered_map<unsigned, long> idIdx;
BPT scoreBPT;
...
bool insert(char* name, unsigned* ID, float* score, char* dept) { 
    ...
    if (!file || !idxfile) { cerr << "student.dat or score.idx file open error" << endl; return false; }
    if (idIdx.find(*ID) != idIdx.end()) return false; 
...
bool searchByID(char* name, unsigned ID, float* score, char* dept) {
    ...
    auto it = idIdx.find(ID);
    if (it == idIdx.end()) {return false;}
