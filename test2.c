...
//데이터시트의 Timing Diagram 참고하여 라이브러리 (lcd.c) TODO 부분 완성
static void LCD_WR_REG(uint16_t LCD_Reg) // LCD의 레지스터에 값 쓰는 역할
{
	// TODO implement using GPIO_ResetBits/GPIO_SetBits
    // low로 셋팅 후
    GPIO_ResetBits(GPIOC, GPIO_Pin_8);  // LCD_CS(0);
    GPIO_ResetBits(GPIOD, GPIO_Pin_13);   // LCD_RS(0); RS는 D/C와 동일
    //RS와 DC는 기능적으로는 동일하지만, RS는 문자 LCD, D/C는 그래픽 LCD 모듈에서 주로 사용되는 이름입니다.
...
static void LCD_WR_DATA(uint16_t LCD_Data) // LCD의 데이터 레지스터에 값 쓰는 역할
{
	// TODO implement using GPIO_ResetBits/GPIO_SetBits
    GPIO_ResetBits(GPIOC, GPIO_Pin_8);  // LCD_CS(0);
    GPIO_SetBits(GPIOD, GPIO_Pin_13);   // LCD_RS(1); RS : D/C
    GPIO_ResetBits(GPIOB, GPIO_Pin_14); // LCD_WR(0);
...
//LCD_ShowNum(8, 100, tch_x, 10, GREEN,WHITE);
{
	u8 size = 16;
	u8 t, temp;
	u8 enshow = 0;
