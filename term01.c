...
void USART2_Init(void) {
    ...
    USART2_InitStructure.USART_StopBits = USART_StopBitsm_1;
...
void USART2_IRQHandler() {
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) {
        word = USART_ReceiveData(USART2);   // 모바일에서 입력한 메세지를 읽고
        /*
        우리가 구현할 내용은 건조가 완료된 상태일 때
        usart2의 블루투스 모듈과 연결된 '사용자 휴대폰'에 
        일방향 메세지를 전송해야함으로 main 등 에서 
        USART_SendData(USART2, finished_alarm_msg);로 알림을 보내면 
        USART2_IRQHandler가 RXNE 조건 받고 인터럽트 됨.
        메세지를 화면에 띄우는 건 블투 터미널 앱에서 처리함.
         
        해당 핸들러는 내부 시스템 제어하는 용으로도 쓸 순 있음. 
        예를 들어 블투에서 앱으로 숫자 입력 받아서 모터 수를 제어할 수 있음
        */
        if(word == 'a') {
            motor_num = 1;
        }
...
void LCD_DisplayStatus(uint16_t humidity, uint16_t illuminance) {
    char buffer[32];
    ...
    sprintf(buffer, "Illuminance: %d", illuminance);
...
int main(void) {
    ...
    while (1) {
        ...
        if(!user_mode) {
            ...
            else if (adc_values[1] < 60 && working_state) { // 건조 완료 (습도 60 미만)
                working_state = false;
                MotorOPChange(0);
                sendDataUART2(msg);
        ...
        LCD_DisplayStatus(adc_values[1], adc_values[0]);
        delay(500);
    }
